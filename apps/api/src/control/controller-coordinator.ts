...
// per-controller watermark. Each bound button TOGGLES its whole-home scene:
//   • single.on false → applyHomeScene(sceneId), set single.on = true
//   • single.on true  → applyHomeSceneOff(scene),  set single.on = false
...
async function processController(deviceId: string, online: boolean): Promise<void> {
  ...
  for (const ev of events) {
    ...
    const scene = store.get().homeScenes.find((x) => x.id === single.sceneId);
    if (!scene) {
      console.error(`[scene-controller] btn ${index} single_click -> scene ${single.sceneId} missing — skipped`);
      continue;
    }
    const turnOn = !single.on;
    try {
      if (turnOn) await applyHomeScene(single.sceneId);
      else await applyHomeSceneOff(single.sceneId);
      ...
      console.log(`[scene-controller] btn ${index} single_click -> scene "${scene.name}" on=${turnOn}`);
    } catch (e) {
